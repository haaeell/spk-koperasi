        ...
        <div class="card-header border-0 pt-5">
            @if (!Auth::user()->isOwner())
                <div class="card-toolbar">
                    <a href="{{ route('sub-kriteria.create') }}" class="btn btn-sm fw-bold btn-primary">Tambah Sub Kriteria</a>
                </div>
            @endif
        </div>
                                ...
                                @foreach ($subKriteriaGroup as $subKriteria)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('sub-kriteria.destroy', $subKriteria->id) }}" method="POST" style="display:inline;">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Hapus sub-kriteria?')">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
